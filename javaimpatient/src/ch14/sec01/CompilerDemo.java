...
public class CompilerDemo {
  public static void main(String[] args) throws ReflectiveOperationException {
    ...
    String rectangleCode = makeClass("Rectangle", "Point", "topLeft", "int", "width", "int", "height");
    System.out.println(rectangleCode);

    List<StringSource> sources = List.of(new StringSource("Point", pointCode),
  ...
  public static String makeClass(String name, String... typesAndFields) {
    ...
    for (int i = 0; i < typesAndFields.length; i += 2) {
      ...
      String ufield = field.substring(0, 1).toUpperCase() + field.substring(1);
      result.append(String.format("    private %s %s;\n", type, field));
      result.append(String.format("    public %s get%s() { return %s; }\n", type, ufield, field));
      result.append(
              String.format("    public void set%2$s(%1$s %3$s) { this.%3$s = %3$s; }\n", type, ufield, field));
    }
    result.append("}\n");
